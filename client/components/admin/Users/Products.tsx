import React, { useState, useEffect } from 'react';
import { User} from 'types';
import { UserService } from 'services';
import { ErrorMessage, Pagination, Spinner, Button, Modal } from 'components/ui';
import { FaRegEdit, FaTrash } from 'react-icons/fa';
import { formatPrice } from 'utils/helpers';
import { useToast } from 'contexts';
import styles from './Products.module.css';
import AddUser from '../AddUser';

const LIMIT = 10;

const Users: React.FC = () => {
  const { setToast } = useToast();

  const [products, setProducts] = useState<User[]>([]);
  const [userEdit, setUserEdit] = useState({})
  const [total, setTotal] = useState(0);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [isOpenAddProductModal, setIsOpenAddProductModal] = useState(false);

  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const payload = { params: { page, limit: LIMIT } };
        const results = await UserService.fetchUsers(payload);
        console.log("results", results)
        setProducts(results.users);
        setTotal(results.total);
      } catch (error) {
        setError(error.message);
      } finally {
        setLoading(false);
      }
    };
    fetchProducts();
  }, [page]);

  const onChangePaginate = (val: number) => {
    setPage(val);
  };

  const closeAddProductModal = () => {
    setIsOpenAddProductModal(false);
  };

  const openAddProductModal = () => {
    setIsOpenAddProductModal(true);
  };

  const handleAddProduct = async (newProduct) => {
    try {
      const { product } = await UserService.addUser(newProduct);
      console.log("product", product)
      setProducts([product, ...products]);
      setIsOpenAddProductModal(false);
      setToast('success', 'Successfully added');
    } catch (error) {
      setToast('error', error.message);
    }
  };

  const handleDeleteProduct = async (id: string) => {
    try {
      const isConfirm = window.confirm('Are you sure you want to delete this user?');
      if (!isConfirm) return;
      await UserService.deleteUser(id);
      const filterProducts = products.filter((product) => product._id !== id);
      setProducts(filterProducts);
      setToast('success', 'Successfully deleted');
    } catch (error) {
      setToast('error', error.message);
    }
  };

  const handleEditProduct = async (id: string) => {
    try {
      const userEdit = await UserService.fetchUser(id);
      setUserEdit(userEdit)
      setIsOpenAddProductModal(true);
    } catch (error) {
      setToast('error', error.message);
    }
  };

  if (loading) {
    return <Spinner size={60} />;
  }

  if (error) {
    return <ErrorMessage message="Unable to get products right now please try again later." />;
  }

  return (
    <div className="table-container">
      <Modal visible={isOpenAddProductModal} title="Add User" onClose={closeAddProductModal}>
        <AddUser onSubmit={handleAddProduct} />
      </Modal>
      <div className={styles.addProductContainer}>
        <Button title="Add User" onClick={openAddProductModal} />
      </div>
      {products?.length === 0 ? (
        <div className={styles.msg}> No products created yet. </div>
      ) : (
        <>
          <table className={styles.table}>
            <thead>
              <tr>
                <th style={{ width: '30rem' }}> Name </th>
                <th> Email </th>
                <th> Role </th>
                <th style={{ width: '20rem' }}> Cart number </th>
                <th> Action </th>
              </tr>
            </thead>
            <tbody>
              {products?.map((product) => (
                <tr key={product._id}>
                  <td>
                    <a target="_blank" rel="noreferrer">
                      <div className={styles.productInfo}>
                        <div className={styles.name}> {product.name} </div>
                      </div>
                    </a>
                  </td>
                  <td>
                    <div className={styles.price}>{product.email}</div>
                  </td>
                  <td>
                    <div className={styles.price}>{product.role}</div>
                  </td>
                  <td>
                    <div className={styles.desc}>{product.carts.length}</div>
                  </td>
                  <td>
                    <button
                      className={styles.btnDelete}
                      onClick={() => handleDeleteProduct(product._id)}
                    >
                      <FaTrash size={16} />
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          <Pagination limit={LIMIT} onChange={onChangePaginate} total={total} active={page} />
        </>
      )}
    </div>
  );
};

export default Users;
